...
describe FindNumber do
  ...
  describe '#game_over?' do
    ...
    context 'when guess and random_number are equal' do
      ...
      subject(:game_match) { described_class.new(0, 9, random_number, 7) }

      it 'is game over' do
        expect(game_match).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      subject(:game_no_match) { described_class.new(0, 9, random_number, 3) }
      it 'is not game over' do
        expect(game_no_match).not_to be_game_over
      end
    end
  end
